...
def normalize_attempt(source: str, row: Mapping) -> ToolAttempt:
    ...
    The learning layer consumes this shape, not recorder tables. Host adapters can
    remain different internally without leaking their schemas into analysis code.
    """
    ...
    return ToolAttempt(
        ...
        command=command,
        error=row.get("error") or row.get("output") if status == "failure" else row.get("error"),
